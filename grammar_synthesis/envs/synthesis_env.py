...
class GrammarSynthesisEnv(gymnasium.Env):
    ...
    def __init__(self, grammar: str, reward_fn: Callable[[str], float], max_len: int=100, 
        ...
        self.terminals = [value for key, value in self.grammar.terminals.items() if key not in ('STOP', 'EMPTY')]
        self.nonterminals = [value for key, value in self.grammar.nonterminals.items() if key not in ("S'")]
